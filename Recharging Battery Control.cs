...
namespace Ingame_Scripts.RechargingBatteryControl {
	...
	public class Program : MyGridProgram {
		...
		//----------------------------------------------------------------------------------------------------------------
		private readonly List<IMyBatteryBlock> localBatteries = new List<IMyBatteryBlock>();
		private readonly List<IMyBatteryBlock> connectedBatteries = new List<IMyBatteryBlock>();
		...
		public Program() {
			...
			GridTerminalSystem.GetBlocksOfType<IMyBatteryBlock>(localBatteries, b => b.CubeGrid == Me.CubeGrid);
		}
		...
		public void Main() {
			...
			GridTerminalSystem.GetBlocksOfType<IMyBatteryBlock>(connectedBatteries, b => b.CubeGrid != Me.CubeGrid && !localBatteries.Contains(b));
			foreach (IMyBatteryBlock bat in connectedBatteries) {
				if (bat.CurrentStoredPower < 0.99*bat.MaxStoredPower)
		...
		private int getTotalPercent() {
			float c = 0;
			float s = 0;
			foreach (IMyBatteryBlock bat in localBatteries) {							
